...
class UniformBox:
    ...
    def __post_init__(self):
        # Allow scalar inputs for 1D cases
        if np.asarray(self.S, dtype=object).ndim == 0 and np.asarray(self.P, dtype=object).ndim == 0:
            self.S = [[to_simplest_fraction(self.S), to_simplest_fraction(self.P)]]
            self.P = np.asarray([1.0 / float(self.S[0][1] - self.S[0][0])], dtype=float)
        else:
            # normalize
    ...
    def __str__(self) -> str:
        return f"uniform({self.S},{self.P})"
...
class EventualExp:
    ...
    def integerize_to_eed(self, k: Sequence[int]) -> EED:
        """
